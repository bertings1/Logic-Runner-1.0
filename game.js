/* Full working game.js with requested changes */
...
(() => {
  ...
  const sentenceBank = {
    easy: [
      ...
      { type: 'sentence', sentence: "Either A ___ B (one or both)", choices: ["OR","AND"], answer: "OR" },
  ...
  function scrollToPanel(id){
    ...
    const panel=document.getElementById(id); if(panel) panel.classList.add('active-panel'); panel.scrollIntoView({behavior:'smooth',block:'start'});
    settingsCard.style.display=(id==='home')?'':'none';
    const menuCards=document.getElementById('menu-cards'); if(menuCards) menuCards.style.display=(id==='home')?'':'none';
  ...
  function startRun(){
    ...
    const menuCards=document.getElementById('menu-cards'); if(menuCards) menuCards.style.display='none';
  ...
  function buildQuestions(diff,total){ total=total||6; const matches=buildMatchQuestions(Math.ceil(total*0.35),diff); const sentences=buildSentenceQuestions(Math.ceil(total*0.35),diff); const ttVariants=buildTruthTableQuestions(diff); const ttCount=Math.max(1,total-matches.length-sentences.length); const ttQuestions=ttVariants.slice(0,ttCount).map(t=>({...t,type:'tt',presentation:pickRandom(['drag','mcq','partial','type','dropdown'])})); return shuffle([...matches,...sentences,...ttQuestions]).slice(0,total); }
